...
async function run() {
  ...
  try {
    ...
    for (const t of tables) {
      await sql.unsafe(`DROP POLICY IF EXISTS "Admins can manage all tasks" ON ${t}`).catch(() => {});
      await sql.unsafe(`DROP POLICY IF EXISTS "Admins can manage all submissions" ON ${t}`).catch(() => {});
      await sql.unsafe(`DROP POLICY IF EXISTS "Admins can manage all" ON ${t}`).catch(() => {});
    }
